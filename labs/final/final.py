...
def getListOfPickUpTransformations(Trobot_block):
    ...
    # CASE 2: VERTICAL WHITE SIDE, TOP
    elif worldZ_BlockCoord[2] == 1:
    ...
    # CASE 3: VERTICAL WHITE SIDE, BOTTOM
    else:
...
def pickUpBlock(team, listOfPoses, map, arm):
    #convert listOfPoses into arm configs
    if team == 'red':
        neutral_config = np.array([-0.09129,  0.09964, -0.21542, -1.50576,  0.02128,  1.6031,   2.04984])
    else:
        neutral_config = np.array([0.24331,  0.09761,  0.0523,  -1.5058,  -0.0051,   1.60328,  2.65172])
    q = np.array([-0.09129,  0.09964, -0.21542, -1.50576,  0.02128,  1.6031,   2.04984])
    listOfConfigs = list()
    listOfConfigsWaypoints = list()
    Trobot_EE_success1 = 0
    print("BLAH")
    print(Trobot_EE_success1)
    Trobot_EE_successReturn = 0
    for x in listOfPoses:
        ...
        if success:
            ...
            if Trobot_EE_success1 == 0:
                Trobot_EE_success1 = 1
                Trobot_EE_successReturn = x
        x[2,3] = x[2,3] + 0.075
        print(x)
        qnew, success, rollout = ik.inverse(x,q)
        if success:
            listOfConfigsWaypoints.append(qnew)
    ...
    print(listOfConfigs)
    index = 0
    arm.safe_move_to_position(listOfConfigsWaypoints[0])
    print("At Waypoint Above Block")
    arm.safe_move_to_position(listOfConfigs[0])
    print("At Block")
    #close gripper
    arm.exec_gripper_cmd(0.03,50)
    print("Gripper Closed")

    return index,success,Trobot_EE_successReturn
...
def robotPlaceBlock(team, map, arm, height):
    ...
    target[2,3] = target[2,3] + 0.075
    q2, success, rollout = ik.inverse(target,seed)

    arm.safe_move_to_position(q2)
    print("At Waypoint Above Stack")

    arm.safe_move_to_position(q)
    print("Block Placed")

    # drop the block, 0.075 for 75 mm, 0 for 0 N
    arm.exec_gripper_cmd(0.75, 0)
    print("block released")
    arm.safe_move_to_position(q2)
    print('above block')
    print("Neutral Position Above Goal Platform")
    return target
...
def rotateBlock(team, height, Trobot_block_target):
    #function assumes IK solver will always be successfull
    ...
    # Case on block orientation to see how many rotations to do, if any
    z = np.array([0, 0, 1])
    Rrobot_block = Trobot_block_target[0:3, 0:3]
    blockZ_WorldCoord = np.matmul(Rrobot_block, z)
    blockZ_WorldCoord = roundVect(blockZ_WorldCoord)

    if blockZ_WorldCoord[2] == 0:

        if blockZ_WorldCoord[0] == 1:

            # THIS IS ANDREAS ELSE CODE
            #rotate -90deg about yaxis
            print("Executing")
            #open
            print("open")
            #move to -45
            ...
            # THIS IS ANDREAS IF(A) CODE
            #rotates +90deg about y axis
            print("Executing")
            #open
            print("open")
            #move to -45
...
if __name__ == "__main__":
    ...
    for x in Trobot_block_ordered_list:
        print(x)
    ...
    Trobot_EE_list, Tblock_EE_list = getListOfPickUpTransformations(Trobot_block_ordered_list[3])
    #for x in Trobot_EE_list:
    #    print(x)
    #pickUpBlock(team, Trobot_EE_list,deepcopy(map_struct),arm)
    #robotPlaceBlock(team, map, arm, 3)
    for a in range(4):

        Trobot_block_before = Trobot_block_ordered_list[a]

        Trobot_EE_list, Tblock_EE_list = getListOfPickUpTransformations(Trobot_block_before)
        for x in Trobot_EE_list:
            print(x)

        _,_,Trobot_EE_success1 = pickUpBlock(team, Trobot_EE_list,deepcopy(map_struct),arm)
        print("Trobogt_block_before inverse")
        print(np.linalg.inv(Trobot_block_before))
        print("Trobot_EE_success1")
        print(Trobot_EE_success1)
        Tblock_EE_success1 = np.matmul(np.linalg.inv(Trobot_block_before), Trobot_EE_success1)

        Tworld_EE = robotPlaceBlock(team, map, arm, a)

        Trobot_block_target = np.matmul(Tworld_EE, np.linalg.inv(Tblock_EE_success1))

        rotateBlock(team, a, Trobot_block_target)
