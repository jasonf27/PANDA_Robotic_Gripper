...
def robotPlaceBlock(team, map, current_pose, height):
    """
    ...
    if (team == "blue"):
        disp = np.array([0.562, 0.809, 0.200+0.030+0.055*height])
    else:
        disp = np.array([0.562, -0.809, 0.200+0.030+0.055*height])
    ...
    ik = IK()
    #need seed
    seed = np.array([0, -1, 0, -2, 0, 1.57, 0])
    goal = ik.inverse(H, seed)

    #find path for arm
    print("Starting to plan")
    start = perf_counter()
    path = rrt(deepcopy(map), deepcopy(current_pose), deepcopy(goal))
    stop = perf_counter()
    dt = stop - start
