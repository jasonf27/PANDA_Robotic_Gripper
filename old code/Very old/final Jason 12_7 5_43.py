...
def orderSides(sidePose_list, Rrobot_block):
    ...
    return sides_list_ordered = [x,y for _,x,y in sorted(sides_list)] # Contains block frame post and position
...
def getListOfPickUpTransformations(Trobot_block):
    ...
    for Rblock_EE in Rblock_EE_list:
        ...
        Tblock_EE = np.vstack(np.hstack(Rblock_EE, np.transpose(dBlock_EE_BlockCoord)), np.array([0, 0, 0, 1]))
