...
def moveToPose(current, target, map,arm):
    ...
    #        print(x)
    #    return 0
    if isPathClear(np.array([-0.09129,  0.09964, -0.21542, -1.50576,  0.02128,  1.6031,   2.04984]),target,map):
...
def getBlocks(Trobot_tag_list):
    ...
    :param
        ...
        the robot frame
    :return:
        Trobot_block_list: list of size 4 where each index is a tuple with the upwards facing tag and the
    ...
    for (name, pose) in Trobot_tag_list:
        ...
        Trobot_block_list.append((name,Trobot_block))
...
def orderSides(sidePose_list, Rrobot_block):
    ...
    print(sides_list)
    #print(sorted(sides_list))
    sides_list_ordered = [(x,y) for (a,x,y) in sides_list] # Contains block frame post and position
...
def getListOfPickUpTransformations(Trobot_block):
    ...
    # CASE 2: VERTICAL WHITE SIDE I.E. TOP OR BOTTOM
        ...
        # Cannot possibly get the side bonus


    d = 0 # Distance from block to EE, TO-DO check this!
...
def rotateBlockWhiteUp(Trobot_block):
    Rrobot_block = Trobot_block[1][0:3,0:3]

    if Trobot_block[0] == 'tag1':
        Rblock_EE_init = np.array([
            [0.707, 0, -0.707],
            [0, -1, 0],
            [-0.707, 0, -0.707],
        ])
    else if Trobot_block[0] == 'tag2':
        do something
    else if Trobot_block[0] == 'tag3':
        do something
    else if Trobot_block[0] == 'tag4':
        do something
    else if Trobot_block[0] == 'tag5':
        do something
    else if Trobot_block[0] == 'tag6':
        do something
...
if __name__ == "__main__":
    ...
    print(Trobot_tag_list[0])

    # get a list of the transformations of the static blocks in the robot frame
    Trobot_block_list = getBlocks(Trobot_tag_list)
    print(Trobot_block_list[0])
    print(Trobot_block_list[0][0])
    rotateBlockWhiteUp(Trobot_block_list[0])


    map_struct = loadmap("../../maps/emptyMap.txt")
    # create a new list of theight = 0
    #for Trobot_block in Trobot_block_ordered_list:
    print(Trobot_block_ordered_list[0])
    Trobot_EE_list, Tblock_EE_list = getListOfPickUpTransformations(Trobot_block_ordered_list[0])
    for x in Trobot_EE_list:
        print(x)
        print('\n')
    #goToBlock(Trobot_EE_list,deepcopy(map_struct),arm)
    #robotPlaceBlock(team, map, arm, 0)
