...
def rotateBlock(team, height, Trobot_block):
    #function assumes IK solver will always be successfull
    ...
    z = np.array([0, 0, 1])
    Tblock_robot = np.linalg.inv(Trobot_block)
    Rblock_robot = Tblock_robot[0:3, 0:3]
    worldZ_BlockCoord = np.matmul(Rblock_robot, z)
    worldZ_BlockCoord = roundVect(worldZ_BlockCoord)
...
if __name__ == "__main__":
    ...
    for a in range(4):
        Trobot_EE_list, Tblock_EE_list = getListOfPickUpTransformations(Trobot_block_ordered_list[a])
        for x in Trobot_EE_list:
            print(x)
        pickUpBlock(team, Trobot_EE_list,deepcopy(map_struct),arm)
        robotPlaceBlock(team, map, arm, a)
        rotateBlock(team, a, Trobot_block_ordered_list[a])
